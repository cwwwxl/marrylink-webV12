...
public class HostWalletController {
    ...
    public Result<PageResult<HostWallet>> page(
        ...
        // 填充主持人姓名
        List<HostWallet> records = page.getRecords();
        if (!records.isEmpty()) {
            Set<Long> hostIds = records.stream()
                    .map(w -> w.getHostId())
                    .collect(Collectors.toSet());
            List<Host> hosts = hostService.listByIds(hostIds);
            Map<Long, String> hostNameMap = new java.util.HashMap<>();
            for (Host h : hosts) {
                hostNameMap.put(h.getId(), h.getName());
            }
            for (HostWallet w : records) {
                w.setHostName(hostNameMap.get(w.getHostId()));
            }
        }
